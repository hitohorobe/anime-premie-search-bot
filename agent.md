...
先行上映会のあとに公開される「レポート記事」（開催の様子を報告する記事、開催前の告知ではない）は、LLM抽出時に`is_report`フラグで判定し、`Event.is_report=True`の場合は`is_concluded()`がセッション日時の有無に関係なく常にtrueを返す（`models.py`）。レポート記事はセッション日時が記事に書かれていないことが多く、日時ベースの終了判定だけでは「終了済み」と認識できないため。これにより`pipeline.run_collect`の`if event.is_concluded(): continue`で新規記事として収集されない（既存の「終了済みイベントは無視する」方針と同じ扱い）。
